input = ""
input = "D2FE28"
inputbin = ""
for l in list(input):
    b = bin(int(l, 16))[2:].zfill(4)
    inputbin = inputbin+b

def get_number(inputbin):
    number = ""
    workbin = inputbin
    while True:
        read = workbin[0]
        number = number+workbin[1:5]
        if read == "0":
            break
        workbin = workbin[5:]
    number = int(number, 2)
    return (number, workbin)

while True:
    workbin = inputbin
    version = int(workbin[0:3], 2)
    workbin = workbin[3:]
    id = int(workbin[0:3], 2)
    workbin = workbin[3:]
    indi = workbin[0]
    workbin = workbin[1:]
    if indi == '0':
        sub = workbin[:15]
        sublen = int(sub, 2)
    else:
        sub = workbin[:11]
        sublen = int(sub, 2)
    number, workbin = get_number(workbin)    

number = get_number(inputbin)


print("resut")
